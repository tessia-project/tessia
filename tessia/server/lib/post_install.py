...
class PostInstallChecker(object):
    ...
    def _fetch_systemd_dns(self):
        """
        Extract the DNS servers configured in systemd-resolve
        """
        try:
            resolv_path = self._exec_ansible(
                'command', "realpath /etc/resolv.conf").strip()
        except RuntimeError:
            self._logger.debug("could not resolve symlink of resolv.conf:",
                               exc_info=True)
            return []

        # systemd-resolved not in use: nothing to do
        if resolv_path != "/run/systemd/resolve/stub-resolv.conf":
            return []

        # collect nameservers from resolv.conf generated by systemd
        try:
            file_content = self._exec_ansible(
                'command', 'cat /run/systemd/resolve/resolv.conf')
        except RuntimeError:
            self._logger.debug(
                "could not read /run/systemd/resolve/resolv.conf:",
                exc_info=True)
            return []
        ...
        for line in file_content.splitlines():
            if not line.strip().startswith('nameserver '):
                continue
            dns_entry = line.replace('nameserver ', '', 1).strip()
            # ignore empty value
            if dns_entry:
                servers.append(dns_entry)
